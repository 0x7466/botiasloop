...
RSpec.describe Botiasloop::Conversation do
  describe "validations" do
    ...
    it "allows custom ID" do
      conversation = described_class.new(id: "custom-convo-123")
      expect(conversation.id).to eq("custom-convo-123")
    end

    it "does not require label" do
      conversation = described_class.new
  ...
  describe "label uniqueness" do
    it "allows same label after previous conversation is deleted" do
      label = "project-#{SecureRandom.hex(4)}"
    ...
    it "prevents duplicate labels globally" do
      label = "unique-project-#{SecureRandom.hex(4)}"
